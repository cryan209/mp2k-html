...
document.getElementById('btnPlay').addEventListener('click', async () => {
  ...
  if (document.getElementById('engineSelect')?.value === 'gsf-lle') {
    try {
      ...
      const dmaEntries = diagnostics?.fifo?.dmaLog || [];
      const dmaShown = [1, 2].flatMap(ch => dmaEntries.filter(d => d.ch === ch).slice(-2));
      ...
      const irqTrace = irq?.dispatches?.length
        ? ` irqTrace:[${irq.dispatches.slice(-6).map(d => `${d.result}${d.steps ? `/${d.steps}` : ''}@${d.pcHex}->${d.handlerHex}/${d.pendingHex}${d.reason ? `(${d.reason})` : ''}`).join(' ')}]`
        : '';
      ...
      const psgTriggerStats = diagnostics?.io?.psgTriggerStats?.length
        ...
        : '';
      const psgWaveNoise = diagnostics?.io?.psgWaveState
        ? ` wave:[en=${diagnostics.io.psgWaveState.enabled?1:0}/freq=${diagnostics.io.psgWaveState.freq}/level=${diagnostics.io.psgWaveState.outputLevel}/force=${diagnostics.io.psgWaveState.forceVolume?1:0}] noise:[en=${diagnostics.io.psgNoiseState.enabled?1:0}/vol=${diagnostics.io.psgNoiseState.volume}/div=${diagnostics.io.psgNoiseState.divRatio}/shift=${diagnostics.io.psgNoiseState.shiftFreq}/width=${diagnostics.io.psgNoiseState.widthMode}]`
        : '';
      const noiseTrigLog = diagnostics?.io?.noiseTriggerLog?.length
        ? ` noiseTrig(${diagnostics.io.noiseTriggerCount}):[${diagnostics.io.noiseTriggerLog.map(e => `vol=${e.volInit}/div=${e.div}/w=${e.width}/shift=${e.shift}/gap=${e.gapMs}ms`).join(' ')}]`
        : '';
      const psgFreqLog = diagnostics?.io?.psgFreqLog?.some(l => l.length)
        ? ` psgFreq:[${diagnostics.io.psgFreqLog.map((log, ch) => `ch${ch}(${log.map(e => `${e.freqHz}Hz${e.trigger ? '!' : ''}`).join(',')})`).join(' ')}]`
        : '';
      const waveRamDump = diagnostics?.io?.waveRamHex?.length
        ? ` waveRam(NR30=${diagnostics.io.soundCnt3LHex}):[${diagnostics.io.waveRamHex.join(',')}]`
        : '';
      ...
      const fmtClickEvent = e => `${e.ms}ms/c${e.cycles}:${e.from}->${e.to}(Δ${e.delta}/avg${e.localAvg})${e.dsFrom !== undefined ? ` ds:${e.dsFrom}->${e.dsTo}` : ''}${e.toTrace ? ` src:${e.toTrace.addr !== null ? '0x' + (e.toTrace.addr >>> 0).toString(16) : '?'}/${e.toTrace.writeInfo}/lag${e.toTrace.lagCycles ?? '?'}/stale${e.toTrace.staleCycles ?? 'never'}` : ''}`;
      ...
      const mixTraceSummary = mixTrace.length
        ...
        : '';
      const mixPcm = memPeek?.mixerPcmTrace || [];
      const mixPcmSummary = mixPcm.length
        ? ` | mixPcm(${mixPcm.length}):[${mixPcm.map(e => `${e.ch}@${e.addr}=${e.bytes.join(',')}`).join(' ')}]`
        : '';
      const mixVol = memPeek?.mixVolTrace || [];
      const mixVolSummary = mixVol.length
        ? ` | mixVol(${mixVol.length}):[${mixVol.map(e => `n${e.n}@vbl${e.vbl}:r4=${e.r4},r10=${e.r10},r11=${e.r11}`).join(' ')}]`
        : '';
      const spWatchAddr = memPeek?.spWatchAddr;
      const spWatchLog = memPeek?.spWatchLog || [];
      const spWatchSummary = spWatchAddr
        ? ` | spWatch(${spWatchAddr}, ${spWatchLog.length} writes):[${spWatchLog.map(w => `vbl${w.vbl}:${w.addrHex}=${w.valueHex}@${w.pcHex}/${w.kind}`).join(' ')}]`
        : '';
      const spStoreOperands = memPeek?.spStoreOperands || [];
      const spStoreOperandsSummary = spStoreOperands.length
        ? ` | spStoreOperands(${spStoreOperands.length}):[${spStoreOperands.map(o => `vbl${o.vbl}:r0=${o.r0},r1=${o.r1},r2=${o.r2},[r0+12]=${o.r0plus12}`).join(' ')}]`
        : '';
      const literalAt081dce2c = memPeek?.literalAt081dce2c;
      const literalAt081dce2cSummary = literalAt081dce2c
        ? ` | literal@081dce2c:ptr=${literalAt081dce2c.ptr},byteAtPtr=${literalAt081dce2c.byteAtPtr}`
        : '';
      const literalWatchAddr = memPeek?.literalWatchAddr;
      const literalWatchLog = memPeek?.literalWatchLog || [];
      ...
      const dmaStackOverlapsSummary = dmaStackOverlaps.length
        ? ` | dmaStackOverlaps(${dmaStackOverlaps.length}):[${dmaStackOverlaps.map(d => `vbl${d.vbl}:ch${d.ch}/${d.reason} ${d.srcHex}->${d.dstHex}..${d.dstEndHex} n=${d.count}x${d.width}B`).join(' ')}]`
        : '';
      ...
      const mixGateRuns = [];
      for (const e of mixGate) {
        const last = mixGateRuns[mixGateRuns.length - 1];
        if (last && last.r0 === e.r0 && last.r4 === e.r4) { last.count++; last.vblEnd = e.vbl; }
        else mixGateRuns.push({ r0: e.r0, r4: e.r4, vblStart: e.vbl, vblEnd: e.vbl, count: 1 });
      }
      ...
      const lastHitIrqPcTrace = memPeek?.lastHitIrqPcTrace || [];
      const lastMissIrqPcTrace = memPeek?.lastMissIrqPcTrace || [];
      const irqPcTraceSummary = (lastHitIrqPcTrace.length || lastMissIrqPcTrace.length)
        ? ` | irqPcHit(${lastHitIrqPcTrace.length}):[${lastHitIrqPcTrace.join(',')}] irqPcMiss(${lastMissIrqPcTrace.length}):[${lastMissIrqPcTrace.join(',')}]`
        : '';
      const hvs = memPeek?.handlerVblSpan;
      const handlerVblSpanSummary = hvs
        ? ` | handlerVblSpan:avg=${hvs.count ? (hvs.total / hvs.count).toFixed(2) : 0}/max=${hvs.max}/capped=${hvs.capped}/n=${hvs.count}`
        : '';
